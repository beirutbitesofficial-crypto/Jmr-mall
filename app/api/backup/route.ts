...
export async function GET(request:Request) {
  ...
  const current=snapshot?JSON.parse(snapshot.payload):await data(request).then(r=>r.json());
...
export async function POST(request:Request) {
  ...
  try {
    ...
    const before=await data(request).then(r=>r.json());
    ...
    for(const d of b.departments)q.push(env.DB.prepare('INSERT INTO departments(id,meter_section,category,owner,phone,occupant,occupant_number,rent_start,rent_end,active) VALUES (?,?,?,?,?,?,?,?,?,?)').bind(d.id,...textFields.map(k=>d[k]),d.active));
